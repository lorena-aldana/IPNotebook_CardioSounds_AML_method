...
def predict_next_R_peak(sig, peaks, sr, avgvalue, plot = False):
    ...
    for i in range(Np-1):
        ...
        if i>1:#avg_interval:
            ...
            #print ('rr_time is: %s')%(rr_time) in samples
            ind= np.reshape(xdata,(-1,1)) #Reshape to use scikit learn with 1d dataset
            dep= np.reshape(rr_time,(-1,1)) #Reshape to use scikit learn with 1d dataset            
            x_train, x_test, y_train, y_test = train_test_split(ind, dep, test_size=0.2)
            model.fit(ind, rr_time)
            rpf_lr[i+1] = model.predict(i+1) #Forecast in seconds
            rpf_lr_cumul[i+1] = peaks[i][0] + model.predict(i+1) #Cumulative forecast, includes value of peak, not just RR value
...
def actual_vs_predicted_onset_one_lead(sig, sr=1000.0, window = 200, thpercentage = 0.65, avgvalue=5, fc1 = 0.5, fc2 = 40, plot = False):
    ...
    #Filter signal
    # peaks = find_R_peaks_onechannel(sig, sr, window, thpercentage, plot = False)
    #Filter por peaks in noisy signals
